...
{
    DbKitIcon *icon = [[self alloc] init];
...
+ (UIImage *)imageWithStackedIcons:(NSArray *)icons imageSize:(CGSize)imageSize
{
    ...
    for (DbKitIcon *icon in icons) {
        NSAssert([icon isKindOfClass:[DbKitIcon class]], @"You can only stack FAKIcon derived objects.");
