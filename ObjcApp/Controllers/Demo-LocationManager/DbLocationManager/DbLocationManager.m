...
- (NSDictionary*)location
{
    ...
    NSDictionary *locationDict = @{DB_LATITUDE:[NSNumber numberWithDouble:self.locationManager.location.coordinate.latitude] , DB_LONGITUDE:[NSNumber numberWithDouble: self.locationManager.location.coordinate.longitude] , DB_ALTITUDE:[NSNumber numberWithDouble:self.locationManager.location.altitude]};
    return locationDict;
}
...
- (void)getGeoCodeAtLocation:(CLLocation*)location
{
    ...
    [self.geocoder reverseGeocodeLocation:location completionHandler:^(NSArray* placemarks, NSError* error){
        ...
        if ([placemarks count] > 0)
        {
            ...
            weakSelf.lastKnownGeocodeAddress = @{DB_LATITUDE    : [NSNumber numberWithDouble:location.coordinate.latitude],
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
{
    if (locations && [locations count] >= 1)
    {
        ...
        {
            NSDictionary *locationDict = @{DB_LATITUDE:[NSNumber numberWithDouble:location.coordinate.latitude] , DB_LONGITUDE:[NSNumber numberWithDouble: location.coordinate.longitude] , DB_ALTITUDE:[NSNumber numberWithDouble:location.altitude]};
...
- (void)locationManager:(CLLocationManager *)manager didFailWithError:(NSError *)error
{
    if([self.delegate respondsToSelector:@selector(DbLocationManagerDidUpdateLocation:)])
    {
        [self.delegate DbLocationManagerDidUpdateLocation:@{DB_LATITUDE:[NSNumber numberWithDouble:0.0f] , DB_LONGITUDE:[NSNumber numberWithDouble:0.0f] , DB_ALTITUDE:[NSNumber numberWithDouble:0.0f]}];
    }
    if(self.locationCompletionBlock != nil)
    {
