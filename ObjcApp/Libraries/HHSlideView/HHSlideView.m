...
- (void)addButtons {
    if([_buttonsArray count] >= [self.namesOfSlideItems count]) {
        return;
    }
...
- (void)addContentScrollView {
    ...
    for (UIViewController *vc in self.childControllersArray) {
        ...
        [vc.view setFrame:CGRectMake(index * SCREEN_WIDTH, 100, SCREEN_WIDTH, SCREEN_HEIGHT-SlideView_Height)];
...
- (UIButton *)p_customButtonWithFrame:(CGRect)rect forTitle:(NSString *)title withTag:(NSInteger)tag {
    ...
    [button.titleLabel setFont:[UIFont fontWithName:@"OpenSans-Semibold" size:16.0f]];
...
- (void)setSelected:(NSInteger)tag {
    [self.delegate slideView:self didSelectItemAtIndex:tag];
    [self p_animateSliderWithTag:tag];
